...
function handleHttpResponse(err, res, body, cb) {
  if (err) return cb(err, res.statusCode, body); // added return statusCode + body

  if (ERROR_CODES.indexOf(res.statusCode) !== -1) {
    var message = body;

    if (body.messages) message = JSON.stringify(body);

    // return cb(errors.http(res.statusCode, message));

    return cb(message); // added return of full Readability message
  }
